...
#             # Filter hits with a relevance threshold (example: 0.1)
#             relevant_hits = [h for h in hits if h.get('score', 0) >= 0.1]

#             if not relevant_hits:
#                 print("No sufficiently relevant documents found")
#                 return ("Sorry, I can only assist with queries related to Indian legal documents. "
...
#             context = "\n\n---\n\n".join(context_parts)
#             scores = [f"{h.get('score', 0):.1f}" for h in relevant_hits[:3]]
#             print(f"Found {len(relevant_hits)} relevant docs (scores: {scores})")

#             # System prompt
#             system_prompt = """You are an expert Indian legal assistant. Provide accurate, well-structured legal information based on the provided context from official Indian legal documents.

# Response Structure:
# 1. Clear introduction to the legal topic
# 2. Main legal analysis with proper citations
# 3. Relevant provisions (sections and acts)
# 4. Practical implications if applicable
# 5. Summary of key points

# Formatting:
# 1. Use ## for main sections, ### for subsections
# 2. **Bold** for important legal terms
# 3. Always cite: Section X of Act Name, Year
# 4. Quote exact legal language when appropriate

# Constraints:
# 1. Base answers strictly on provided context
# 2. State limitations if information is incomplete
# 3. Include all relevant provisions mentioned"""
...
# Please provide a comprehensive, professionally structured answer with proper legal citations and formatting."""
